...
# Constantes para os arquivos de credenciais (utilizando caminhos absolutos)
...
# Escopos de acesso para o Google Sheets e Google Docs
SHEETS_SCOPES = [
...
def extract_questions_from_doc(document_id: str) -> list:
    ...
    """
    try:
        # Carrega as credenciais e constrói o serviço da API do Google Docs
        ...
        # Processa o conteúdo do documento, extraindo perguntas e respostas
        for item in content:
            if "paragraph" in item:
                ...
                # Identifica opções de resposta (formato "(a) ...")
...
def write_to_spreadsheet(questions: list, spreadsheet_url: str):
    ...
    """
    try:
        # Carrega as credenciais e abre a planilha
        ...
        # Obtém as perguntas existentes na planilha (primeira coluna)
        existing_questions = sheet.col_values(1)[1:] 

        new_questions = []
        # Verifica se há novas perguntas a serem adicionadas (evita duplicatas)
        ...
        if new_questions:
            ...
            for question in new_questions:
                ...
                # Remove tags HTML e formata as perguntas para a planilha
                question = [
                    item.replace("<b>", "").replace("</b>", "") for item in question
                ]
                question.extend([""] * (6 - len(question)))  # Completa com linhas vazias
...
def monitor_google_docs(
    ...
    while True:
        try:
            # Obtém as credenciais, cria o serviço e busca o documento
            credentials = service_account.Credentials.from_service_account_file(
                DOCS_CREDENTIALS_FILE, scopes=DOCS_SCOPES
            )
            service = build("docs", "v1", credentials=credentials)
